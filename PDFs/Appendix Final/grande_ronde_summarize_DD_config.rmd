---
title: '<span style=''font-size: 20px''><b>14. Cumulative degree days summary results for Grande Ronde River summer steelhead under year 2017 temperatures for the Columbia River with simulated additional coldwater refuges</b></style>'
author: " "
date: " "
...
C14<-fread(paste(fishdir,"gr_Columbia2017B_minus1/combined/E14.csv", sep=""), header=TRUE, sep=",")
D14<-fread(paste(fishdir,"GR_config_test/combined/E14.csv", sep=""), header=TRUE, sep=",")
...
```{r plot DD hist together, fig.cap="Fig. 14.1 Histograms of modeled Grande Ronde River summer steelhead accumulated degrees day over 18°C from Bonneville to the Snake River confluence in the Columbia River."}
...
```{r cum DD boxplot, fig.cap="Fig. 14.2 Boxplots of modeled Grande Ronde River summer steelhead accumulated degrees day over 18°C from Bonneville to the Snake River confluence in the Columbia River."}
...
DD_used_table$scenario[DD_used_table$scenario == "A"] <- "Columbia 2017, CWRs Current"
...
kable(DD_used_table, col.names=c( "Scenario", "Minimum", "25% quantile","Median", "75% quantile", "Maximum"),row.names=FALSE, caption="Table 14.1 Cumulative degree days (>18°C) used across different HexSim thermalscapes summarized for Grande Ronde River summer steelhead.", digits=1)
```
...
```{r plot DD hist together GT 20, fig.cap="Fig. 14.3 Histograms of modeled Grande Ronde River summer steelhead accumulated degrees day over 20°C from Bonneville to the Snake River confluence in the Columbia River."}
plot_grid(a2,d2, c2, 
          labels = c("A",  "B", "C"),
          ncol = 3, nrow = 1)
```

```{r cum DD boxplot GT 20, fig.cap="Fig. 14.4 Boxplots of modeled Grande Ronde River summer steelhead accumulated degrees day over 20°C from Bonneville to the Snake River confluence in the Columbia River."}
all_14_ddGT18$scenario<-as.factor(all_14_ddGT18$scenario)
...
# create table with pretty format
kable(DD_used_table, col.names=c( "Scenario", "Minimum", "25% quantile","Median", "75% quantile", "Maximum"),row.names=FALSE, caption="Table 14.2 Cumulative degree days (>20°C) used across different HexSim thermalscapes summarized for Grande Ronde River summer steelhead.", digits=1)
```
...
```{r plot DD hist together GT 21, fig.cap="Fig. 14.5 Histograms of modeled Grande Ronde River summer steelhead accumulated degrees day over 21°C from Bonneville to the Snake River confluence in the Columbia River."}
plot_grid(a2,  d2,c2,
          labels = c("A",  "B","C"),
          ncol = 3, nrow = 1)
```

```{r cum DD boxplot GT 21, fig.cap="Fig. 14.6 Boxplots of modeled Grande Ronde River summer steelhead accumulated degrees day over 21°C from Bonneville to the Snake River confluence in the Columbia River."}
all_14_ddGT18$scenario<-as.factor(all_14_ddGT18$scenario)
...
# create table with pretty format
kable(DD_used_table, col.names=c( "Scenario", "Minimum", "25% quantile","Median", "75% quantile", "Maximum"),row.names=FALSE, caption="Table 14.3 Cumulative degree days (>21°C) used across different HexSim thermalscapes summarized for Grande Ronde River summer steelhead.", digits=1)
```
