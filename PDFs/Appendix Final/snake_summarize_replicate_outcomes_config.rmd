---
title: '<span style=''font-size: 20px''><b>17. Energy use, CWR use, and survivorship results for Snake River Fall Chinook Salmon under year 2017 temperatures for the Columbia River with simulated additional coldwater refuges</b></style>'
author: " "
date: " "
...
# SR fall chinook
A14<-fread(paste(fishdir,"snake_Columbia2017B/combined/E4.csv", sep=""),header=TRUE, sep=",")
#B_SR<-fread(paste(fishdir,"snake_Columbia2017B_NoCWR/combined/E4.csv", sep=""),header=TRUE, sep=",")
C14<-fread(paste(fishdir,"snake_Columbia2017B_minus_1/combined/E4.csv",sep=""), header=TRUE, sep=",")
D14<-fread(paste(fishdir,"config_test_srchnk/combined/E4.csv",sep=""), header=TRUE, sep=",")
...
```{r plot energy hist together, fig.cap="Fig. 17.1 Histogram of percent energy lost for modeled Snake River Fall Chinook Salmon migrating through four different modeled thermalscapes."}
plot_grid( a1, d1, c1,
          labels = c("A",  "B", "C"),
          ncol = 3, nrow = 1)
```
...
```{r energy lost boxplot, fig.cap="Fig. 17.2 Boxplot of percent energy lost for modeled Snake River Fall Chinook Salmon migrating through four different modeled thermalscapes."}
...
kable(ED_used_table, col.names=c( "Scenario", "Minimum", "25% quantile","Median", "75% quantile", "Maximum"),row.names=FALSE, caption="Table 17.1 Percent energy used across different HexSim thermalscapes summarized for Snake River Fall Chinook Salmon.", digits=1)
```
...
# import in combined replicate DFs
A9<-fread(paste(fishdir,"snake_Columbia2017B/combined/E9.csv", sep=""), header=TRUE, sep=",")
#B14<-fread(paste(fishdir,"gr_Columbia2017B_NoCWR/combined/E14.csv", sep=""), header=TRUE, sep=",")
C9<-fread(paste(fishdir,"snake_Columbia2017B_minus_1/combined/E9.csv", sep=""), header=TRUE, sep=",")
D9<-fread(paste(fishdir,"config_test_srchnk/combined/E9.csv", sep=""), header=TRUE, sep=",")
...
kable(refuge_use_all, digits=0, col.names=c("Scenario", "CWR Residence (h/individual)"), caption="Table 17.2 Model output for hours residing in cold water refuges summarized for Snake River Fall Chinook Salmon.")
```
...
kable(acute_all,digits=2, col.names=c("Scenario", "Total mortality"), caption="Table 17.3 Model output for percent of individuals dying from acute temperature stress summarized for Snake River Fall Chinook Salmon.")
