---
title:  '<span style=''font-size: 20px''><b>4. Energy use, CWR use, and survivorship results for Grande Ronde River summer steelhead under long-term average temperatures for the Columbia River </b></style>'
author: " "
date: " "
...
```{r plot energy hist together, fig.cap="Fig. 4.1 Histogram of percent energy lost for modeled Grande Ronde summer steelhead migrating through different modeled thermalscapes."}
...
```{r energy lost boxplot, fig.cap="Fig. 4.2 Boxplot of percent energy lost for modeled Grande Ronde summer steelhead migrating through different modeled thermalscapes."}
...
kable(ED_used_table, col.names=c( "Scenario", "Minimum", "25% quantile","Median", "75% quantile", "Maximum"),row.names=FALSE, caption="Table 4.1 Percent energy used across different HexSim thermalscapes summarized for Grande Ronde River Summer Steelhead.", digits=1)
```
...
kable(refuge_use_all, digits=0, col.names=c("Scenario", "CWR Residence (h/individual)"), caption="Table 4.2 Model output for hours residing in cold water refuges summarized for Grande Ronde River Summer Steelhead.")
```
...
acute_E<-((dim(E12)[1])/ 6000) * 100
acute_F<-((dim(F12)[1])/ 6000) * 100
acute_G<-((dim(G12)[1])/ 6000) * 100
acute_v<-c(acute_C, acute_G, acute_A, acute_E,acute_B, acute_F)
acute_all<-cbind.data.frame(names_v, acute_v)
kable(acute_all,digits=2, col.names=c("Scenario", "Total mortality"), caption="Table 4.3 Model output for percent of individuals dying from acute temperature stress summarized for Grande Ronde River Summer Steelhead.")

```



```{r time in system histogram, eval=TRUE}
...
```{r terminus arrival histogram for current w and without CWR, eval=TRUE}
# subset data to the two scenarios
all_13_labels_2<-filter(all_13_labels, scenario == "Columbia Current, CWR Current" | scenario == "Columbia Current, No CWRs")
all_13_labels_2$scenario<-factor(all_13_labels_2$scenario)

# histograms
# p <- ggplot(data = all_13_labels_2, aes(x = date)) + geom_histogram()
# p + facet_wrap(~scenario, ncol=1) + labs(x="Arrival date at end of modeled reach", y="Count", title="Grande Ronde River Summer Steelhead") + theme_classic()
```



```{r terminus arrival histogram for current w and without CWR 2, eval=TRUE}
# subset data to the two scenarios
all_13_labels_2<-filter(all_13_labels, scenario == "Columbia Current, CWR Current" | scenario == "Columbia Current, No CWRs")
all_13_labels_2$scenario<-factor(all_13_labels_2$scenario)

# histograms
p <- ggplot(data = all_13_labels_2, aes(x = date, y = 100*(..count../sum(..count..)))) + geom_histogram()
p + facet_wrap(~scenario, ncol=1) + labs(x="Arrival date at end of modeled reach", y="Percent", title="Grande Ronde River Summer Steelhead") + theme_classic()
```
...
```{r plot of passage days for current current scenario, eval=FALSE}
columbia_current_cwr_current<-dplyr::filter(all_13_labels, scenario == "Columbia Current, CWR Current")
e<-ggplot(data=columbia_current_cwr_current, aes(x=Time_in_system_d)) + geom_histogram(binwidth=2) + xlim(0,260) + labs(x="Modeled Passage time (days from Bonneville to Snake River confluence)")
```


```{r plot of passage days for current scenario with keefer observed data, eval=FALSE, fig.cap="Fig. 7. Histograms of modeled Grande Ronde River summer steelhead passage days from Bonneville to the Snake River confluence in the Columbia River and observed passage days from Bonneville to McNary summarized from pit tag data (Keefer)."}
plot_grid(d,e,
          labels = c("A", "B"),
          ncol = 1, nrow = 2)
```

```{r plot of mcnary passage date observed, eval=FALSE}
...
```{r confluence passage date modeled, eval=FALSE}
all_13_labels_current<-filter( all_13_labels, scenario == "Columbia Current, CWR Current" )

# histograms
z <- ggplot(data = all_13_labels_current, aes(x = date)) + geom_histogram(bins=325) + labs(x="Modeled Passage Date (Snake River confluence)", y="Count") + scale_x_datetime(limits = ymd_hms(c("2019-06-01 17:00:00", "2020-04-20 17:00:00")))
z 
```

```{r confluence passage date hour modeled, eval=FALSE}
all_13_labels_current$Time_leaving<-all_13_labels_current$Time_leaving + all_13_labels_current$Time_in_system

# histograms
z <- ggplot(data = all_13_labels_current, aes(x = Time_entry)) + geom_histogram(bins=250) + labs(x="Modeled Passage Date (Snake River confluence)", y="Count") 
z 
```

```{r combine modeled and observed passage date plots, eval=FALSE}
plot_grid(f,z,
          labels = c("A", "B"),
          ncol = 1, nrow = 2)
```
...
```{r energy lost boxplot2, eval=FALSE, fig.cap="Fig. X. Boxplot of percent energy lost for modeled Grande Ronde summer steelhead migrating through different modeled thermalscapes."}
all_13$scenario<-as.factor(all_13$scenario)

all_13_6<-filter(all_13, scenario != "D")

out2<-ggplot(all_13_6, aes(x=scenario, y=ED_per_used, group=scenario)) +   
  geom_boxplot() + 
  labs( y="Energy Density Lost (%)", title="Grande Ronde River Summer Steelhead") + 
  theme_classic() 

out2 + theme(axis.text.x=element_text(color = "black", size=11, angle=30, vjust=.8, hjust=0.8), axis.title.x=element_blank(),) + scale_x_discrete(limits=c("B", "F","C","G", "A","E"), labels=c(
  "Columbia Historic, \nCWR Current", 
 "Columbia Historic, \nNo CWRs",
  "Columbia Current,\n CWR Current",
  "Columbia Current, \nNo CWRs", 
