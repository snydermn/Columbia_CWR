---
title: "tucannon summarize fitness outcomes"
author: "MNSnyder"
date: "8/19/2019"
...
```{r A13 hist}
a1<-ggplot(A13, aes(x=ED_per_used)) + geom_histogram(aes(x=ED_per_used, y=100 * (..count..)/sum(..count..))) + xlim(0,15) + ylim(0,20) + theme_classic() + theme(legend.position="none", axis.text=element_text(size=text1),
        axis.title=element_text(size=text1)) + labs(x="Energy Used (%)", y="Percent", title="Columbia 2040, CWR Current") 
```

```{r B13 hist}
b1<-ggplot(B13, aes(x=ED_per_used)) + geom_histogram(aes(x=ED_per_used, y=100 * (..count..)/sum(..count..))) + xlim(0,15) + ylim(0,20) + theme_classic() + theme(legend.position="none", axis.text=element_text(size=text1),
        axis.title=element_text(size=text1)) + labs(x="Energy Used (%)", y="Percent", title="Columbia Restored, CWR Current")
```

```{r C13 hist}
c1<-ggplot(C13, aes(x=ED_per_used)) + geom_histogram(aes(x=ED_per_used, y=100 * (..count..)/sum(..count..))) + xlim(0,15) + ylim(0,20) + theme_classic() + theme(legend.position="none", axis.text=element_text(size=text1),
        axis.title=element_text(size=text1)) + labs(x="Energy Used (%)", y="Percent", title="Columbia Current, CWR Current")
```

```{r D13 hist}
d1<-ggplot(D13, aes(x=ED_per_used)) + geom_histogram(aes(x=ED_per_used, y=100 * (..count..)/sum(..count..))) + xlim(0,15) + ylim(0,20) + theme_classic() + theme(legend.position="none", axis.text=element_text(size=text1),
        axis.title=element_text(size=text1)) + labs(x="Energy Used (%)", y="Percent", title="Columbia Current, CWR Restored")
```

```{r plot energy hist together, fig.cap="Fig. 1. Histogram of percent energy lost for modeled tucannon summer steelhead migrating through four different modeled thermalscapes."}
...
```{r energy lost boxplot, fig.cap="Fig. 2. Boxplot of percent energy lost for modeled tucannon summer steelhead migrating through four different modeled thermalscapes."}
...
```{r plot DD hist together, fig.cap="Fig. 3. Histograms of modeled Tucannon River summer steelhead accumulated degrees day over 18°C from Bonneville to the Snake River confluence in the Columbia River."}
...
```{r cum DD boxplot, fig.cap="Fig. 4. Boxplots of modeled Tucannon River summer steelhead accumulated degrees day over 18°C from Bonneville to the Snake River confluence in the Columbia River."}
