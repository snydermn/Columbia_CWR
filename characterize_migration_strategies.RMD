...
```{r run biological metric functions on end condition data}
...
```{r run biological metric from hourly data function on hourly current current}

biological2<-hourly_biological_f(E14_2)
```

```{r combine 3 metric DFs}
metric1<-merge(biological1, biological2, by.x="ID", by.y="ID", all=TRUE)
metric2<-merge(metric1, thermal_out, by.x="ID", by.y="ID")
```
...
```{r created energy used without gametes}
...
metric3<-merge(metric2, A_energy, by.x="ID", by.y="ID")
```
