...
```{r select a few fish IDs for testing}
...
```{r generate average daily stats from hourly input}
...
##########################################################
############################################################
######### try with full data set ############################


```{r try with full dataset}
...
date_time_df<-cbind.data.frame(hour_seq, date_time)
# merge with hourly DF
E14_sub2<-merge(E14, date_time_df, by.x="Step", by.y="hour_seq")
# drop unnecessary columns
Col.keep<-c("ID", "temp_1", "date_time")
all_E<-select(E14_sub2, ID, temp_1, date_time)

```


```{r thermal exposure metrics with full dataset, eval=FALSE}
...
```{r calculate thermal exposure metrics from functions}
...
```{r create function to run all functions and merge metric DFs}
thres_hot <- 21
thres_cold <- 18
