...
```{r write out DF for Matt K}
write.csv(hourly_and_end_A, "D:/git_repo/Columbia_CWR/summary_output/grande_ronde_exposure_summary_per_indiv_110519.csv")
```


```{r A plot duration vs energy used}
# get mean for 3 color plot
#mid<-mean(hourly_and_end_A$Total_hours_cold_water)
# plot
dur1<-ggplot(randomssubset, aes(x=Time_in_system_d, y=ED_per_used, color=Total_hours_cold_water)) +   
  geom_point() + 
  labs( y="Energy lost (%)", x="Time migrating (d)", title="Grande Ronde River Summer Steelhead") + 
  theme_classic() +  scale_color_gradientn(name="Hours CWR (total)", colours = rainbow(9))
...
dur1
#  
# + scale_color_gradient(name="Hours in Cold Water", low="blue", mid="yellow",high="red")
```

```{r B plot duration vs energy used}
dur2<-ggplot(randomssubset, aes(x=Time_in_system_d, y=ED_per_used, color=temp_1)) +   
  geom_point() + 
  labs( y="Energy lost (%)", x="Time migrating (d)", title="Grande Ronde River Summer Steelhead") + 
  theme_classic() + scale_color_gradientn(name="Mean temperature", colours = rainbow(9))
```

```{r C plot duration vs energy used}
dur3<-ggplot(randomssubset, aes(x=Time_in_system_d, y=ED_per_used, color=entry_jday)) +   
  geom_point() + 
  labs( y="Energy lost (%)", x="Time migrating (d)", title="Grande Ronde River Summer Steelhead") + 
  theme_classic() + scale_color_gradientn(name="Entry date", colours = rainbow(9))
```

```{r D plot duration vs energy used}
dur4<-ggplot(randomssubset, aes(x=Time_in_system_d, y=ED_per_used, color=terminus_jday)) +   
  geom_point() + 
  labs( y="Energy lost (%)", x="Time migrating (d)", title="Grande Ronde River Summer Steelhead") + 
  theme_classic() + scale_color_gradientn(name="Exit date", colours = rainbow(9))
```
...
```{r time in system vs time in CWR contour plot}
data.loess <- loess(ED_per_used ~ Time_in_system_d * Total_hours_cold_water, data = hourly_and_end_A)
xgrid <-  seq(min(hourly_and_end_A$Time_in_system_d), max(hourly_and_end_A$Time_in_system_d), 2)
length(xgrid)
ygrid <-  seq(min(hourly_and_end_A$Total_hours_cold_water), max(hourly_and_end_A$Total_hours_cold_water), 30)
length(ygrid)
# Generate a dataframe with every possible combination of wt and hp
data.fit <-  expand.grid(Time_in_system_d = xgrid, Total_hours_cold_water = ygrid)
...
zlim1<-min(hourly_and_end_A$ED_per_used)
zlim2<-max(hourly_and_end_A$ED_per_used)
contour(x = xgrid, y = ygrid, z = mtrx3d, zlim=c(zlim1, zlim2), labcex = 1.2, xlab = "Time migrating (d)", ylab ="Time in CWR (h)")
```

```{r time in system vs time in CWR contour plot3}
data.loess <- loess(ED_per_used ~ Total_hours_cold_water * Time_in_system_d, data = hourly_and_end_A)
Ygrid <-  seq(min(hourly_and_end_A$Time_in_system_d), max(hourly_and_end_A$Time_in_system_d), 2)
length(xgrid)
Xgrid <-  seq(min(hourly_and_end_A$Total_hours_cold_water), max(hourly_and_end_A$Total_hours_cold_water), 30)
length(ygrid)
# Generate a dataframe with every possible combination of wt and hp
data.fit <-  expand.grid( Total_hours_cold_water = Xgrid, Time_in_system_d = Ygrid)
# Feed the dataframe into the loess model and receive a matrix output with estimates of
# acceleration for each combination of wt and hp
mtrx3d <-  predict(data.loess, newdata = data.fit)
zlim1<-min(hourly_and_end_A$ED_per_used)
zlim2<-max(hourly_and_end_A$ED_per_used)
contour(y = Ygrid, x = Xgrid, z = mtrx3d, zlim=c(zlim1, zlim2), labcex = 1.2, ylab = "Time migrating (d)", xlab ="Time in CWR (h)")
```


```{r time in system vs time in CWR contour plot2}
data.loess <- loess(Total_hours_cold_water ~ Time_in_system_d * ED_per_used, data = hourly_and_end_A)
xgrid <-  seq(min(hourly_and_end_A$Time_in_system_d), max(hourly_and_end_A$Time_in_system_d), 2)
length(xgrid)
ygrid <-  seq(min(hourly_and_end_A$ED_per_used), max(hourly_and_end_A$ED_per_used), 1)
length(ygrid)
# Generate a dataframe with every possible combination of wt and hp
data.fit <-  expand.grid(Time_in_system_d = xgrid, ED_per_used = ygrid)
# Feed the dataframe into the loess model and receive a matrix output with estimates of
# acceleration for each combination of wt and hp
mtrx3d <-  predict(data.loess, newdata = data.fit)
zlim1<-min(hourly_and_end_A$Total_hours_cold_water)
zlim2<-max(hourly_and_end_A$Total_hours_cold_water)
contour(x = xgrid, y = ygrid, z = mtrx3d, zlim=c(zlim1, zlim2), labcex = 1.2, xlab = "Time migrating (d)", ylab ="Energy Used (%)")
```


```{r time in system vs temp contour plot}
data.loess <- loess(ED_per_used ~ Time_in_system_d * temp_1, data = hourly_and_end_A)
xgrid <-  seq(min(hourly_and_end_A$Time_in_system_d), max(hourly_and_end_A$Time_in_system_d), 2)
length(xgrid)
ygrid <-  seq(min(hourly_and_end_A$temp_1), max(hourly_and_end_A$temp_1), 0.5)
length(ygrid)
# Generate a dataframe with every possible combination of wt and hp
data.fit <-  expand.grid(Time_in_system_d = xgrid, temp_1 = ygrid)
# Feed the dataframe into the loess model and receive a matrix output with estimates of
# acceleration for each combination of wt and hp
mtrx3d <-  predict(data.loess, newdata = data.fit)
zlim1<-min(hourly_and_end_A$ED_per_used)
zlim2<-max(hourly_and_end_A$ED_per_used)
contour(x = xgrid, y = ygrid, z = mtrx3d, zlim=c(zlim1, zlim2),labcex = 1.2, xlab = "Time migrating (d)", ylab ="Mean temperature")
```

```{r time in system vs temp contour plot2}
data.loess <- loess( temp_1~ Time_in_system_d * ED_per_used, data = hourly_and_end_A)
xgrid <-  seq(min(hourly_and_end_A$Time_in_system_d), max(hourly_and_end_A$Time_in_system_d), 2)
length(xgrid)
ygrid <-  seq(min(hourly_and_end_A$ED_per_used), max(hourly_and_end_A$ED_per_used), 2)
length(ygrid)
# Generate a dataframe with every possible combination of wt and hp
data.fit <-  expand.grid(Time_in_system_d = xgrid, ED_per_used = ygrid)
# Feed the dataframe into the loess model and receive a matrix output with estimates of
# acceleration for each combination of wt and hp
mtrx3d <-  predict(data.loess, newdata = data.fit)
zlim1<-min(hourly_and_end_A$temp_1)
zlim2<-max(hourly_and_end_A$temp_1)
contour(x = xgrid, y = ygrid, z = mtrx3d, zlim=c(zlim1, zlim2),labcex = 1.2, xlab = "Time migrating (d)", ylab ="Energy used (%)")
```

```{r mean temp vs CWR hours contour plot}
data.loess <- loess(ED_per_used ~ Total_hours_cold_water * temp_1, data = hourly_and_end_A)
xgrid <-  seq(min(hourly_and_end_A$Total_hours_cold_water), max(hourly_and_end_A$Total_hours_cold_water), 2)
length(xgrid)
ygrid <-  seq(min(hourly_and_end_A$temp_1), max(hourly_and_end_A$temp_1), 0.5)
length(ygrid)
# Generate a dataframe with every possible combination of wt and hp
data.fit <-  expand.grid(Total_hours_cold_water = xgrid, temp_1 = ygrid)
# Feed the dataframe into the loess model and receive a matrix output with estimates of
# acceleration for each combination of wt and hp
mtrx3d <-  predict(data.loess, newdata = data.fit)
zlim1<-min(hourly_and_end_A$ED_per_used)
zlim2<-max(hourly_and_end_A$ED_per_used)
contour(x = xgrid, y = ygrid, z = mtrx3d, zlim=c(zlim1, zlim2), xlab = "Time in CWR (h)", ylab ="Mean temperature")
```

```{r time in system vs entry date contour plot}
data.loess <- loess(ED_per_used ~ Time_in_system_d * entry_jday, data = hourly_and_end_A)
xgrid <-  seq(min(hourly_and_end_A$Time_in_system_d), max(hourly_and_end_A$Time_in_system_d), 2)
length(xgrid)
ygrid <-  seq(min(hourly_and_end_A$entry_jday), max(hourly_and_end_A$entry_jday), 10)
length(ygrid)
# Generate a dataframe with every possible combination of wt and hp
data.fit <-  expand.grid(Time_in_system_d = xgrid, entry_jday = ygrid)
# Feed the dataframe into the loess model and receive a matrix output with estimates of
# acceleration for each combination of wt and hp
mtrx3d <-  predict(data.loess, newdata = data.fit)
zlim1<-min(hourly_and_end_A$ED_per_used)
zlim2<-max(hourly_and_end_A$ED_per_used)
contour(x = xgrid, y = ygrid, z = mtrx3d, zlim=c(zlim1, zlim2),labcex = 1.2, xlab = "Time migrating (d)", ylab ="Entry Jday")
```

```{r time in system vs entry date contour plot2}
data.loess <- loess(entry_jday ~ Time_in_system_d * ED_per_used, data = hourly_and_end_A)
xgrid <-  seq(min(hourly_and_end_A$Time_in_system_d), max(hourly_and_end_A$Time_in_system_d), 2)
length(xgrid)
ygrid <-  seq(min(hourly_and_end_A$ED_per_used), max(hourly_and_end_A$ED_per_used), 2)
length(ygrid)
...
# acceleration for each combination of wt and hp
mtrx3d <-  predict(data.loess, newdata = data.fit)
zlim1<-min(hourly_and_end_A$entry_jday)
zlim2<-max(hourly_and_end_A$entry_jday)
contour(x = xgrid, y = ygrid, z = mtrx3d, zlim=c(zlim1, zlim2),labcex = 1.2, xlab = "Time migrating (d)", ylab ="Energy used (%)")
```

```{r time in system vs exit date contour plot}
data.loess <- loess(ED_per_used ~ Time_in_system_d * terminus_jday, data = hourly_and_end_A)
xgrid <-  seq(min(hourly_and_end_A$Time_in_system_d), max(hourly_and_end_A$Time_in_system_d), 2)
length(xgrid)
ygrid <-  seq(min(hourly_and_end_A$terminus_jday), max(hourly_and_end_A$terminus_jday), 10)
length(ygrid)
# Generate a dataframe with every possible combination of wt and hp
data.fit <-  expand.grid(Time_in_system_d = xgrid, terminus_jday = ygrid)
# Feed the dataframe into the loess model and receive a matrix output with estimates of
# acceleration for each combination of wt and hp
mtrx3d <-  predict(data.loess, newdata = data.fit)
zlim1<-min(hourly_and_end_A$ED_per_used)
zlim2<-max(hourly_and_end_A$ED_per_used)
contour(x = xgrid, y = ygrid, z = mtrx3d, zlim=c(zlim1, zlim2),labcex = 1.2, xlab = "Time migrating (d)", ylab ="Exit Jday")
```

```{r time in system vs exit date contour plot2}
data.loess <- loess( terminus_jday~ Time_in_system_d * ED_per_used, data = hourly_and_end_A)
xgrid <-  seq(min(hourly_and_end_A$Time_in_system_d), max(hourly_and_end_A$Time_in_system_d), 2)
length(xgrid)
ygrid <-  seq(min(hourly_and_end_A$ED_per_used), max(hourly_and_end_A$ED_per_used), 2)
length(ygrid)
...
# acceleration for each combination of wt and hp
mtrx3d <-  predict(data.loess, newdata = data.fit)
zlim1<-min(hourly_and_end_A$terminus_jday)
zlim2<-max(hourly_and_end_A$terminus_jday)
contour(x = xgrid, y = ygrid, z = mtrx3d, zlim=c(zlim1, zlim2),labcex = 1.2, xlab = "Time migrating (d)", ylab ="Energy Used (%)")
```

```{r entry date vs exit date contour plot}
data.loess <- loess(ED_per_used ~ entry_jday * terminus_jday, data = hourly_and_end_A)
xgrid <-  seq(min(hourly_and_end_A$entry_jday), max(hourly_and_end_A$entry_jday), 10)
length(xgrid)
ygrid <-  seq(min(hourly_and_end_A$terminus_jday), max(hourly_and_end_A$terminus_jday), 10)
length(ygrid)
# Generate a dataframe with every possible combination of wt and hp
data.fit <-  expand.grid(entry_jday = xgrid, terminus_jday = ygrid)
# Feed the dataframe into the loess model and receive a matrix output with estimates of
# acceleration for each combination of wt and hp
mtrx3d <-  predict(data.loess, newdata = data.fit)
zlim1<-min(hourly_and_end_A$ED_per_used)
zlim2<-max(hourly_and_end_A$ED_per_used)
contour(x = xgrid, y = ygrid, z = mtrx3d, zlim=c(zlim1, zlim2), xlab = "Entry Jday", ylab ="Exit Jday")
```
...
```{r plot winners and losers 1, eval=FALSE, fig.height = 8, fig.width = 6, fig.cap="Fig. 6. "}
...
```{r plot winners and losers 2, eval=FALSE, fig.height = 8, fig.width = 6, fig.cap="Fig. 7. "}
...
```{r plot winners and losers 3, eval=FALSE, fig.height = 8, fig.width = 6, fig.cap="Fig. 8. "}
...
```{r plot winners and losers 4, eval=FALSE, fig.height = 8, fig.width = 6, fig.cap="Fig. 9. "}
plot_grid( step_refuge_win2, step_refuge_loser2, 
          labels = c("A", "B"),align="v",
          ncol = 1, nrow = 2)
```

```{r terminus arrival histogram observed, eval=TRUE}
...
#d <- ggplot(data = psg_timing_gr_keefer, aes(x = bo_mn_d)) + geom_histogram(binwidth=2) + xlim(0,260) + labs(x="Observed Passage time (days from Bonneville to McNary)")
```

```{r plot of mcnary passage date observed, eval=TRUE}
...
f <- ggplot(data = psg_timing_gr_keefer, aes(x = mc_date_4_plot)) + geom_histogram(bins=90)  + labs(x="Observed Passage date (McNary Dam)", title="Grande Ronde River Summer Steelhead") +  scale_x_datetime(limits = ymd_hms(c("2019-06-01 17:00:00", "2020-04-20 17:00:00")))
f
```

```{r set limits for plot axis}
xlim1<-180
xlim2<-300
ylim2<-300
ylim1<-0
...
```{r start date vs time in system for modeled, eval=TRUE}
all_13_labels_current<-dplyr::filter( all_13_labels, scenario == "Columbia 2017, CWR Current" )
c<-ggplot(all_13_labels_current, aes(x=entry_jday, y=Time_in_system_d)) + 
  geom_point() + 
  xlim(xlim1, xlim2) + ylim(ylim1,ylim2) +
  labs(x="Entry Date", y="Days migrating (BO to Snake)") + 
  theme_classic()
c
```

```{r start date vs time in system for observed, eval=TRUE}
d<-ggplot(psg_timing_gr_keefer, aes(x=BO_jul, y=bo_mn_d)) + 
  geom_point() + 
  xlim(xlim1, xlim2) + ylim(ylim1,ylim2) +
  labs(x="Entry Date", y="Days migrating (BO to MC)") + 
  theme_classic()
d
```

```{r plot start date vs passage time for observed and modeled, eval=TRUE, fig.height = 8, fig.width = 6, fig.cap="Fig. 9. "}
plot_grid( c, d, 
          labels = c("", ""),align="v",
          ncol = 1, nrow = 2)
```
