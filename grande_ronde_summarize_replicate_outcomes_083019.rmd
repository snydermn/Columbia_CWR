...
kable(refuge_use_all, col.names=c("Scenario", "Total CWR Residence (h)"), caption="Table 3. Model output for total hours residing in cold water refuges summarized for Grande Ronde River Summer Steelhead.")
```
...
```{r terminus arrival histogram for current w and without CWR}
...
```{r plot of passage days for current current scenario, eval=TRUE}
...
```{r plot of passage days for current scenario with keefer observed data, eval=TRUE, fig.cap="Fig. 7. Histograms of modeled Grande Ronde River summer steelhead passage days from Bonneville to the Snake River confluence in the Columbia River and observed passage days from Bonneville to McNary summarized from pit tag data (Keefer)."}
plot_grid(d,e,
          labels = c("A", "B"),
          ncol = 1, nrow = 2)
```
...
```{r confluence passage date modeled, eval=TRUE}
...
```{r confluence passage date hour modeled, eval=TRUE}
...
```{r combine modeled and observed passage date plots}
plot_grid(f,z,
          labels = c("A", "B"),
          ncol = 1, nrow = 2)
```
...
```{r energy lost boxplot2, eval=TRUE, fig.cap="Fig. X. Boxplot of percent energy lost for modeled Grande Ronde summer steelhead migrating through different modeled thermalscapes."}
all_13$scenario<-as.factor(all_13$scenario)
