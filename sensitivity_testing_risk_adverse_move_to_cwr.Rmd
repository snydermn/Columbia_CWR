---
title: 'Sensitivity testing: risk adverse move to CWR'
author: "MNSnyder"
date: "1/29/2020"
...
To evaluate uncertainty around behavioral thermoregulation behavior influences fish fitness outcomes we ran a Sensitivity analysis of how move to cold water refuge curve, in the Set Movement Behavior events, affects fish fitness metrics. We changed the curve by a parabola shaped factor (0.005, 0.01, 0.06) depending on the model week. We modeled the Grande Ronde Summer Steelhead population because of their large range in propensity to behaviorally thermoregulate. Sensitivity tests were based on a typical recent temperature year (2017) for the Columbia River. The effect of increasing mortality (decreasing survivorship) from temperature stress was measured on fish fitness outcomes including the percent mortality and overall energy used.  
...
B_end<-read.table("D:/Results_4Populations/gr_behavior_test._005/combined/E13.csv", header=TRUE, sep=",")
C_end<-read.table("D:/Results_4Populations/gr_behavior_test_.01/combined/F13.csv", header=TRUE, sep=",")
D_end<-read.table("D:/Results_4Populations/gr_behavior_test_.06/combined/G13.csv", header=TRUE, sep=",")
...
B_end$scenario<-"Factor 0.005"
C_end$scenario<-"Factor 0.01"
D_end$scenario<-"Factor 0.06"


# combine DFs 
all_GR<-rbind.data.frame(A_end, B_end, C_end, D_end)
```
...
B_dead<-read.table("D:/Results_4Populations/gr_behavior_test._005/combined/E12.csv", header=TRUE, sep=",")
C_dead<-read.table("D:/Results_4Populations/gr_behavior_test_.01/combined/F12.csv", header=TRUE, sep=",")
D_dead<-read.table("D:/Results_4Populations/gr_behavior_test_.06/combined/G12.csv", header=TRUE, sep=",")

names_v<-c("Default","Factor 0.005","Factor 0.01","Factor 0.06")
...
summary_GR$scenario <- factor(summary_GR$scenario, levels=c("Default","Factor 0.005","Factor 0.01","Factor 0.06"))
...
ggplot(summary_GR, aes(x=scenario, y=ED_per_used, colour=scenario)) +
  ...
  scale_colour_manual(values=c("blue4", "green", "darkorange3", "purple"))
